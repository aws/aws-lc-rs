...
/// `u64::MAX`.
///
/// At most 2^64 nonces can be generated from a single `PredictableNonceSequence`.
...
impl PredictableNonceSequenceBuilder {
    ...
    /// Generates a random context and position for a `PredictableNonceSequence`.
    /// #  Panics
...
impl PredictableNonceSequence {
    ...
    /// Provides the limit on the number of nonces it's allowed to generate.
    #[must_use]
...
/// `u16::MAX`.
///
/// To minimize the probability of nonces in the sequence being repeated, at most 2^16 nonces can be
/// generated from a single `UnpredictableNonceSequence`.
#[allow(clippy::module_name_repetitions)]
...
impl UnpredictableNonceSequenceBuilder {
    ...
    /// Sets the "context" for the `UnpredictableNonceSequence`.
    #[must_use]
