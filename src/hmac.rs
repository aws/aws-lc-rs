...
impl Drop for LcHmacCtx {
    fn drop(&mut self) {
        unsafe { aws_lc::HMAC_CTX_cleanup(self.as_mut_ptr()) }
    }
}
...
impl Clone for LcHmacCtx {
    fn clone(&self) -> Self {
        unsafe {
            ...
            aws_lc::HMAC_CTX_init(hmac_ctx.as_mut_ptr());
            let mut hmac_ctx = hmac_ctx.assume_init();
            aws_lc::HMAC_CTX_copy_ex(&mut hmac_ctx, self.as_ptr());
            LcHmacCtx(hmac_ctx)
        }
    }
}
