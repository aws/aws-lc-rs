...
pub(super) struct DigestContext(aws_lc::EVP_MD_CTX);

impl DigestContext {
    pub fn new(algorithm: &'static Algorithm) -> Result<DigestContext, Unspecified> {
        let evp_md_type = match_digest_type(&algorithm.id);
        let mut dc = MaybeUninit::<aws_lc::EVP_MD_CTX>::uninit();
        unsafe {
            aws_lc::EVP_MD_CTX_init(dc.as_mut_ptr());
            if 1 != aws_lc::EVP_DigestInit_ex(dc.as_mut_ptr(), *evp_md_type, null_mut()) {
    ...
    pub(super) fn as_mut_ptr(&mut self) -> *mut aws_lc::EVP_MD_CTX {
        &mut self.0
    }

    pub(super) fn as_ptr(&self) -> *const aws_lc::EVP_MD_CTX {
        &self.0
    }
}
...
impl Drop for DigestContext {
    fn drop(&mut self) {
        unsafe {
            aws_lc::EVP_MD_CTX_cleanup(self.as_mut_ptr());
        }
    }
}
...
impl DigestContext {
    fn try_clone(&self) -> Result<Self, &'static str> {
        ...
        unsafe {
            aws_lc::EVP_MD_CTX_init(dc.as_mut_ptr());
            if 1 != aws_lc::EVP_MD_CTX_copy(dc.as_mut_ptr(), self.as_ptr()) {
