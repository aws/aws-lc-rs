...
use aws_lc::{
    ...
    EVP_aead_aes_256_gcm, EVP_aead_chacha20_poly1305, EVP_AEAD, EVP_AEAD_CTX,
};
...
impl AeadCtx {
    pub(crate) fn aes_128_gcm(key_bytes: &[u8]) -> Result<Self, Unspecified> {
        ...
        unsafe {
            let aead = ConstPointer::new(EVP_aead_aes_128_gcm())?;
    ...
    pub(crate) fn aes_256_gcm(key_bytes: &[u8]) -> Result<Self, Unspecified> {
        ...
        unsafe {
            let aead = ConstPointer::new(EVP_aead_aes_256_gcm())?;
    ...
    pub(crate) fn chacha20(key_bytes: &[u8]) -> Result<Self, Unspecified> {
        ...
        unsafe {
            let aead = ConstPointer::new(EVP_aead_chacha20_poly1305())?;
