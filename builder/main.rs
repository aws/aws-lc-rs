...
static mut SYS_CFLAGS: String = String::new();
...
fn initialize() {
    unsafe {
        ...
        SYS_CFLAGS = optional_env_optional_crate_target("CFLAGS").unwrap_or_default();
...
fn setup_include_paths(out_dir: &Path, manifest_dir: &Path) -> PathBuf {
    ...
    if let Some(extra_paths) = get_env_includes_path() {
        include_paths.extend(extra_paths.clone());
    }
